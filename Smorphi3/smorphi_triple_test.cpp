...
// void Smorphi::BeginSmorphi_encoder()
// {
...
//   // void Smorphi::interruptA_M1();
//   // void Smorphi::interruptA_M2();
//   // void Smorphi::interruptA_M3();
//   // void Smorphi::interruptA_M4();
...
void Smorphi_triple::BeginSmorphiTriple()
{
...
void Smorphi_triple::set_interrupt_pin(){
...
int Smorphi_triple::module1_sensor_status(int pin_no){
  int pin_status;
...
int Smorphi_triple::module2_sensor_status(int pin_no){
  int pin_status;
...
int Smorphi_triple::module3_sensor_status(int pin_no){
  int pin_status;
...
int Smorphi_triple::module1_sensor_interrupt(){
...
double Smorphi_triple::mapPosRanges(int ipSpeed) //map user's input linear velocity to robot's actual positive linear velocity
{
  double mappedSpeed = ipSpeed * (sm_max_linear_speed) / 100;
  return mappedSpeed;
}

double Smorphi_triple::mapNegRanges(int ipSpeed) //map user's input linear velocity to robot's actual negative linear velocity
{
  double mappedSpeed = ipSpeed * (-sm_max_linear_speed) / 100;
  return mappedSpeed;
}

double Smorphi_triple::mapPosAng(int ipSpeed) //map user's input angular velocity to robot's actual positive angular velocity
{
  double mappedSpeed = ipSpeed * (sm_max_angular_speed) / 100;
  return mappedSpeed;
}

double Smorphi_triple::mapNegAng(int ipSpeed) //map user's input angular velocity to robot's actual negative angular velocity
{
  double mappedSpeed = ipSpeed * (-sm_max_angular_speed) / 100;
  return mappedSpeed;
}

int Smorphi_triple::map_lv_PWM(double ipSpeed) //map robot's linear velocity to PWM
{
  if (ipSpeed == 0)
  {
    int mappedPWM = 0;
...
int Smorphi_triple::map_ang_PWM(double ipSpeed) //map robot's angular velocity to PWM
{
  if (ipSpeed == 0)
  {
    int mappedPWM = 0;
...
int Smorphi_triple::map_turn_PWM(double ipSpeed) //map robot's curve velocity to PWM
{
  if (ipSpeed == 0)
  {
    int mappedPWM = 0;
...
int Smorphi_triple::sm_feedback(int solenoid_feedback){ 

  if (solenoid_feedback == 2) {
...
void Smorphi_triple::sm_solenoid_set(int solenoid_unlock, bool status){ 

  if (solenoid_unlock == 2){
...
char Smorphi_triple::sm_getShape(){ 
  if ( sm_feedback(2)==1 && sm_feedback(3)==0 && sm_feedback(4)==1 && sm_feedback(5)==0 ){
    robot_shape = 'i';
  }
  ...
  // }
   ...
   else if (sm_feedback(2)==1 && sm_feedback(3)==1 && sm_feedback(4)==0 && sm_feedback(5)==0){
     robot_shape = 'o';
   }
...
void Smorphi_triple::sm_velocity_handler(float sm_req_linear_speed_x, float sm_req_linear_speed_y, float sm_req_angular_speed)
{
  ...
  // //Module 1 
  //   sm_lv_M1FL  =  -( -1.3813*v_x - 0.3035*v_y + ( 0.2459 - sm_wheel_x - sm_wheel_y) * w ); 
  //   sm_lv_M1FR  =  -(  0.3035*v_x - 1.3813*v_y - ( 0.2145 - sm_wheel_x - sm_wheel_y) * w ); 
  //   sm_lv_M1RL  =  -(  0.3035*v_x - 1.3813*v_y - ( 0.2145 + sm_wheel_x + sm_wheel_y) * w ); 
  //   sm_lv_M1RR  =  -( -1.3813*v_x - 0.3035*v_y + ( 0.2459 + sm_wheel_x + sm_wheel_y) * w ); 

  //   // Module 2
  //   sm_lv_M2FL  =  ( v_x - v_y - ( sm_wheel_x + sm_wheel_y) * w );
  //   sm_lv_M2FR  =  ( v_x + v_y + ( sm_wheel_x + sm_wheel_y) * w );
  //   sm_lv_M2RL  =  ( v_x + v_y - ( sm_wheel_x + sm_wheel_y) * w );
  //   sm_lv_M2RR  =  ( v_x - v_y + ( sm_wheel_x + sm_wheel_y) * w );

  //   // Module 3
  //   sm_lv_M3FL  =  -( -1.3930*v_x - 0.2442*v_y + ( 0.1971 - sm_wheel_x - sm_wheel_y) * w );
  //   sm_lv_M3FR  =  -(  0.2442*v_x - 1.3930*v_y + ( 0.2575 + sm_wheel_x + sm_wheel_y) * w );
  //   sm_lv_M3RL  =  -(  0.2442*v_x - 1.3930*v_y + ( 0.2575 - sm_wheel_x - sm_wheel_y) * w );
  //   sm_lv_M3RR  =  -( -1.3930*v_x - 0.2442*v_y + ( 0.1971 + sm_wheel_x + sm_wheel_y) * w );

  //   // Module 4
  //   sm_lv_M4FL  =  -( -1.3930*v_x - 0.2442*v_y + ( 0.4147 - sm_wheel_x - sm_wheel_y) * w );
  //   sm_lv_M4FR  =  -(  0.2442*v_x - 1.3930*v_y + ( 0.3597 + sm_wheel_x + sm_wheel_y) * w );
  //   sm_lv_M4RL  =  -(  0.2442*v_x - 1.3930*v_y + ( 0.3597 - sm_wheel_x - sm_wheel_y) * w );
  //   sm_lv_M4RR  =  -( -1.3930*v_x - 0.2442*v_y + ( 0.4147 + sm_wheel_x + sm_wheel_y) * w );
...
void Smorphi_triple::sm_pwm_handler(int flag)
{
    ...
    double sm_PWMs[3][4] = {{sm_PWM_M1FL, sm_PWM_M1FR, sm_PWM_M1RL, sm_PWM_M1RR},
...
void Smorphi_triple::SetSmorphiSpeed(double pwm[3][4])
{
...
void Smorphi_triple::MotorDirection(){

  double sm_lvs[3][4] = {{sm_lv_M1FL,sm_lv_M1FR,sm_lv_M1RL,sm_lv_M1RR},
  ...
  Adafruit_DCMotor *sm_motors[3][4] = {{M11,M12,M13,M14},
                      {M21,M22,M23,M24},
                      {M31,M32,M33,M34}
                      // {M41,M42,M43,M44}
                      };
...
void Smorphi_triple::MoveForward(const int Speed)
{
...
void Smorphi_triple::MoveBackward(const int Speed)
{
...
void Smorphi_triple::MoveRight(const int Speed)
{
  double vel_speed = mapNegRanges(Speed);
...
void Smorphi_triple::MoveLeft(const int Speed)
{
  double vel_speed = mapPosRanges(Speed);
  sm_velocity_handler(0, vel_speed, 0);
  int flag = 0;
...
void Smorphi_triple::MoveDiagUpRight(const int Speed)
{
...
void Smorphi_triple::MoveDiagUpLeft(const int Speed)
{
...
void Smorphi_triple::MoveDiagDownRight(const int Speed)
{
...
void Smorphi_triple::MoveDiagDownLeft(const int Speed)
{
...
void Smorphi_triple::MoveTurnUpRight(const int Speed, const int angular_velocity)
{
...
void Smorphi_triple::MoveTurnUpLeft(const int Speed, const int angular_velocity)
{
...
void Smorphi_triple::MoveTurnDownRight(const int Speed, const int angular_velocity)
{
...
void Smorphi_triple::MoveTurnDownLeft(const int Speed, const int angular_velocity)
{
  double vel_speed = mapNegRanges(Speed);
  double ang_speed = mapPosAng(angular_velocity);
  sm_velocity_handler(vel_speed, 0, ang_speed);
...
void Smorphi_triple::stopSmorphi()
{ 
...
void Smorphi_triple::CenterPivotLeft(const int angular_velocity)
{
...
void Smorphi_triple::CenterPivotRight(const int angular_velocity)
{
  double ang_speed = mapNegAng(angular_velocity);
  sm_velocity_handler(0, 0, ang_speed);
  int flag = 2;
...
void Smorphi_triple::sm_reset_M1()
{
  sm_lv_M1FL  =  0;
  sm_lv_M1FR  =  0;
  sm_lv_M1RL  =  0;
...
void Smorphi_triple::sm_reset_M2()
{
  sm_lv_M2FL  =  0;
  sm_lv_M2FR  =  0;
  sm_lv_M2RL  =  0;
...
void Smorphi_triple::sm_reset_M3()
{
  sm_lv_M3FL  =  0;
  sm_lv_M3FR  =  0;
  sm_lv_M3RL  =  0;
...
//interrupt testing
void Smorphi_triple::interrupt_test()
{
  if(!digitalRead(INT_PIN_1))
  {
